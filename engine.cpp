...
struct registry {
    ...
    //std::unordered_map<Entity, SpriteComponent> spriteComponents;
    //std::unordered_map<Entity, CollisionBoxComponent> colliderComponents;
};
...
void renderRectShape(SDL_Renderer* renderer, registry& reg) {
    for (int entity = 1; entity <= max_entity; entity++) {
        if (reg.transformComponents.find(entity) != reg.transformComponents.end()) {
            auto& transform = reg.transformComponents[entity];
